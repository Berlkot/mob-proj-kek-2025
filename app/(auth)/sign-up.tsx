...
export default function SignUp() {
  ...
  const signUp = async () => {
    ...
    // Если подтверждение почты отключено, data.session будет существовать.
    // AuthProvider в _layout.tsx сам увидит новую сессию и перекинет нас на (main).
    // Нам не нужно делать router.replace, но для надежности можно проверить:
    
    if (data.session) {
      // Успешный вход без подтверждения
      // Ничего не делаем, AuthProvider сработает автоматически
    } else {
      // Если вдруг подтверждение почты все еще включено
      Alert.alert('Почти готово', 'Проверьте почту для подтверждения (если вы не отключали эту опцию).');
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.btn} onPress={signUp} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Зарегистрироваться и Войти</Text>}
      </TouchableOpacity>
      ...
      <TouchableOpacity 
        ...
        style={{marginTop: 20}}
      >
        <Text style={{color: '#007AFF', textAlign: 'center'}}>Уже есть аккаунт? Войти</Text>
      </TouchableOpacity>
    </View>
  );
}
